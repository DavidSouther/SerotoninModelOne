...
class GlutGSDReceptor(PostSynapticReceptor):
    def __init__(self, targetNeuron, weight, tau):
        ...
        self.C_d = 0.001 # was 0.001 for non-pharma
        ...
        self.tau_p = 30 #ms
        self.c_p = 0.43 # was 4.3 for non-pharma 1 second epoch
    ...
    def postSynapticSpikeFeedback(self):
        # Add to Drive Factor Statistic
        if 0 < self.time - self.lastSpikeTime <= 40:
            self.tempDriveFactor += halfRectify(self.g_SD_nmda - self.th)*exp((self.lastSpikeTime - self.time) / self.tau_p)

        # If we spiked in the last 40 ms, modify theW weight
