...
class Axon():
    ...
    def updateProximalDiffuseTransmitters(self, proximalDiffuseTransmitterLevels):
        ...
        for proximalAxonReceptor in self.proximalDiffuseReceptors:  # For each proximal receptor
            try:
                proximalAxonReceptor.setLevel(self.proximalDiffuseTransmitters[proximalAxonReceptor.getTypeString()]) # Set the level to be whatever matches it in the passed dictionary
            except KeyError:
    ...
    def updateDistalDiffuseTransmitters(self, distalDiffuseTransmitters):
        ...
        for distalAxonReceptor in self.distalDiffuseTransmitters:  # For each distal receptor
            try:
